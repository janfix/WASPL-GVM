    ...
    <div class="container">
      ...
      <div v-if="publicationStore?.publicationData?.mode !=='exam'" class="MainResultContainer">
        <p><b>You have answered {{ allAnswers.length }} questions on {{ allQuestions }} available.</b>
          <br>
          Your total score: <b>{{ totalScore }} on {{ totalMaxScore }} -> {{((totalScore/totalMaxScore)*100).toFixed(2)}}%</b>
        </p>
        <p class="message" v-if="hasShortAnswerQuestions">
          At least one question needs a human or AI correction. <br>
          This version of WASPL does not include the AI module for final scoring. <br>
          The score will be partial (no score here for open-ended questions)   
          </p>
      </div>
  
      <div 
      v-if="publicationStore?.publicationData?.mode !=='exam'"
      v-for="answer in allAnswers" :key="answer.questionId" class="cardReport row">
    ...
    <div class="p-2 text-center" style="font-size: 0.8rem;"> 
      Human correction is required.
        ...
        <div class="col-9">
          ...
          <div
  v-if="answer.score === 'AI' && showCorrectionMap[answer.questionId] && results.correction.length > 0"
>
  <div class="accordion" :id="'accordion-' + answer.questionId">
    ...
    <div class="accordion-item">
      ...
      <div
        :id="'collapseCorrection-' + answer.questionId"
        class="accordion-collapse collapse"
        :aria-labelledby="'headingCorrection-' + answer.questionId"
      >
        <div class="accordion-body">
          <div
            v-for="(correction, index) in results.correction"
            :key="index"
            class="card mb-3"
          >
            <div class="card-body">
    ...
    <div class="accordion-item">
      ...
      <div
        :id="'collapseParams-' + answer.questionId"
        class="accordion-collapse collapse"
