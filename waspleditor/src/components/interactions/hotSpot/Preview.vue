  ...
  <div v-if="selectedElement" class="card card-preview">
    ...
    <div class="card-body">
      ...
      <div v-if="selectedElement?.el_RichText?.trim().length > 0" class="quill-editor" ref="quillEditor"></div>
       <p v-else-if="selectedElement.hasOwnProperty('el_RichText') && !selectedElement?.el_RichText?.trim()">No additional text provided.</p>
      ...
      <div hidden class="mt-3">
        <button  v-if="displayHintButton && !showReport && !showHint" @click="HandlerShowHint" class="btn btn-primary btn-sm">Show Hint</button>
...
.card {
    ...
    overflow: hidden; /* Crucial to hide anything overflowing the card */
}
...
.card-body {
    ...
    padding: 15px;
    width: 100%; /* Make card-body take full width of its parent */
...
#container {
  display: flex;
  flex-direction: row; /* Canvas and panel side-by-side */
  ...
  align-self: center; /* Center the container if card-body allows */
  flex-wrap: nowrap; /* Prevent wrapping */
  position: relative; /* Parent for absolute positioning of emoticon-source */
}
...
#emoticon-source {
  ...
  top: 0;
  left: 0;
  width: 100px; /* Fixed width for the panel */
  max-height: 100%; /* Take max height available from parent (#container) */
  padding: 10px;
  border: 1px solid rgba(204, 204, 204, 0.5); /* Semi-transparent border */
  background-color: rgb(148 148 148 / 60%); /* Semi-transparent dark background */
  display: flex;
  flex-direction: column; /* Stack emoticons vertically */
  align-items: center; /* Center emoticons horizontally */
  user-select: none;
  cursor: default;
  z-index: 6; /* Ensure panel is above the canvas layers and remove zone */
  overflow-y: auto; /* Add scroll if many stickers exceed height */
  box-sizing: border-box;
}
...
.draggable-emoticon {
  ...
  color: white; /* Make emoticons visible on dark background */
}
...
.konva-container {
  width: 100%; /* Konva container takes full width of its parent */
  height: 100%; /* Konva container takes full height of its parent */
  overflow: hidden;
  position: relative;
}
...
.konva-container canvas {
  ...
  height: auto !important; /* Maintain aspect ratio when scaling down */
  display: block; /* Remove extra space below canvas */
  margin: 0 auto; /* Center the canvas if it's smaller than container */
  border: none;
  /* Make sure the canvas background matches the intended background */
  background-color: transparent; /* Konva draws the background, so container bg is not needed */
}
...
.canvas-reset-btn {
  position: absolute;
  top: 10px;
  right: 10px; /* Adjusted position to not overlap with panel edge */
  z-index: 10; /* Ensure it's above the canvas and emoticon panel */
...
.quill-editor, #score  {
    ...
    max-width: 780px; /* Approx 800px container - 2*10px padding/margin */
    box-sizing: border-box;
    align-self: center; /* Center if max-width is less than 100% */
}
...
@media (max-width: 600px) {
    ...
    #emoticon-source {
        ...
        border-top: 1px solid rgba(204, 204, 204, 0.5); /* Horizontal border above */
        background-color: #f8f9fa; /* Lighter background when not overlaid */
        display: flex; /* Keep flex */
        flex-direction: row; /* Layout emoticons horizontally */
        flex-wrap: wrap; /* Allow emoticons to wrap */
        justify-content: center;
        z-index: auto; /* Remove specific z-index */
        padding: 10px 0; /* Adjust padding */
        box-shadow: inset 0 5px 5px -5px rgba(0,0,0,0.1); /* Add a subtle shadow top */
    }
    .draggable-emoticon {
        margin: 5px; /* Adjust margin for horizontal layout */
        color: #333; /* Darker color on lighter background */
    }
     /* Canvas wrapper takes full width when stacked */
     .canvas-wrapper {
        width: 100%;
        flex-grow: 0; /* Don't grow beyond content needs */
         min-height: 150px; /* Adjust min-height for small screens */
     }
      /* Konva container takes full width */
      .konva-container {
         width: 100%;
         height: auto; /* Allow height to adjust based on canvas aspect ratio */
         overflow: hidden;
      }
